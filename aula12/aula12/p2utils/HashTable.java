...
public class HashTable<E> {
  ...
  public String[] keys() {
	String[] arrayKeysList = new String[15];
	String[] arrayKeysFinal = new String[size + 100];
	int size = 0;
	for (int i = 0; i < array.length; i++) {
		arrayKeysList = array[i].keys();
		for (int j = 0; j < arrayKeysList.length; j++) {
			arrayKeysFinal[size] = arrayKeysList[j];
			size += 1; 
		}
	}
	String[] arrayKeysFinalV2 = new String[size];
	for (int x = 0; x < size; x++) {
		arrayKeysFinalV2[x] = arrayKeysFinal[x];
	}
	return arrayKeysFinalV2;
  }
}
